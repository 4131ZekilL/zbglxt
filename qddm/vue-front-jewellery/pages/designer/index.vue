                  ...
                  <li v-for="item in data.items" :key="item.id">
                      <section class="i-teach-wrap">
                          <div class="i-teach-pic">
                             <a :href="'/designer/'+item.id" :title="item.name">
                                  <img :src="item.avatar" :alt="item.name" height="142" hright="142">
                              </a>
                          </div>
                          <div class="mt10 hLh30 txtOf tac">
                              <a :href="'/designer/'+item.id" :title="item.name" class="fsize18 c-666">{{ item.name }}</a>
                          </div>
...
<div>
 ...
 export default {
   asyncData({ params, error }) {
     return designer.getPageList(1, 8).then(response => {
   ...
   methods: {
    gotoPage(page){
        designer.getPageList(page, 8).then(response => {
